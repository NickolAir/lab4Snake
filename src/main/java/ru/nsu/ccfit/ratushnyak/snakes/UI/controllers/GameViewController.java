...
import ru.nsu.ccfit.ratushnyak.snakes.view.GamePresenter;
import ru.nsu.ccfit.ratushnyak.snakes.view.event.ExitEvent;
...
import ru.nsu.ccfit.ratushnyak.snakes.view.event.NewGameEvent;
...
public class GameViewController implements View {
    ...
    private EventHandler<KeyEvent> getMovementEventHandler() {
        return event -> {
            if (gamePresenter == null) {
                throw new IllegalStateException("Cant move with undefined view");
            }
    ...
    private void builtField() {
        if (gameConfig == null) {
            throw new IllegalStateException("Cant create field without configuration");
        }
